(ns robert.test.model.user-test
  (:require [monger.core :refer [connect! get-db set-db!]]
            [monger.collection :as mc]
            
;;            [monger.test.manage-db :as manage]
;;            [monger.data.data :as data]

            [robert.model.user :as user]))

(connect!)
;; (set-db! (get-db (:name data/main-document)))


;; (manage/eliminate-document (:name data/main-document))
;; (manage/create-document (:name data/main-document))
;; (manage/create-first-users (:name data/main-document))

